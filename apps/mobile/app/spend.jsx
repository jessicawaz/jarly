...
export default function Spend() {
  ...
  const { fetchUser, budget } = useUserStore();

  const [amount, setAmount] = useState(0);
  const [jarSelected, setJarSelected] = useState(null);
  const [spendLabel, setSpendLabel] = useState(null);
  ...
  const validateSpend = () => {
    if (!jarSelected) {
      return "Select a jar to log spend.";
    }
    if (!amount) {
      return "Enter amount to log spend.";
    }
  ...
  const logSpend = async () => {
    ...
    try {
      ...
      // Spent % for selectedJar
      const filteredSpends = spends.filter((s) => s.jar === jarSelected);
      const totalSpent =
      ...
      const jarWarning = spendPct >= 80;

      let goalMilestone = false;
      let filteredGoal;
      if (selectedGoal) {
        // fetch goals, find goal
        const { data: goals } = await get("/api/v1/goals");
        filteredGoal = goals.find((g) => g.id === selectedGoal);

        goalMilestone =
          filteredGoal?.savedCents >= filteredGoal?.targetCents;
      }

      if (jarWarning) {
        return Alert.alert(
          "",
          `Heads up! Your ${jarSelected} Jar is almost full!`,
          [{ text: "OK", onPress: () => router.push("/home") }],
        );
      } else if (goalMilestone) {
        return Alert.alert(
          "",
          `Congrats! ${filteredGoal.name} Goal is finished!`,
          [{ text: "OK", onPress: () => router.push("/home") }],
        );
      }
  ...
  return (
    <>
      ...
      >
        ...
        <TouchableOpacity style={styles.button} onPress={logSpend}>
